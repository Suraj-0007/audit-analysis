...
class BrowserManager:
    ...
    async def initialize(self):
        ...
        # Launch browser - we'll create contexts as needed
        self._browser = await self._playwright.chromium.launch(
            headless=False,  # Always start non-headless for login capability
            args=[
    ...
    async def create_authenticated_context(self, session_id: str) -> BrowserContext:
        ...
        # Create new context with saved state - can run headless for audit
        context = await self._browser.new_context(
            viewport={"width": 1280, "height": 720},
            user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
            ignore_https_errors=True,
